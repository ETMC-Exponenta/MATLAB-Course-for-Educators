classdef MATLABCourseForEducatorsDev < handle
    ...
    methods
        function obj = MATLABCourseForEducatorsDev(extender)
            % Init
            if nargin < 1
                obj.ext = MATLABCourseForEducatorsExtender;
            else
                if ischar(extender) || isStringScalar(extender)
                    obj.ext = MATLABCourseForEducatorsExtender(extender);
                else
                    obj.ext = extender;
                end
            end
        ...
        function check(obj)
            ...
            upd = MATLABCourseForEducatorsUpdater(obj.ext);
            upd.fetch();
